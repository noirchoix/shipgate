...
class AuditService:
    ...
    def _static_findings(self, files: dict[str,str], session: dict[str,Any], target: str):
        ...
        for p,txt in files.items():
            if re.search(r'(sk_live_|sk_test_|AKIA[0-9A-Z]{16}|AIza[0-9A-Za-z_-]{20,}|ghp_[0-9A-Za-z_]{30,}|github_pat_[0-9A-Za-z_]{20,})', txt):
            ...
            if re.search(r'\b(?:NEXT_PUBLIC_|VITE_|PUBLIC_)[A-Z0-9_]*(?:SECRET|TOKEN|KEY|PASSWORD)[A-Z0-9_]*\b', txt, re.I):
                f.append(self._finding('blocker','security_auditor','Potential secret exposed through public frontend environment variable',p,'A public-prefixed frontend environment variable appears to contain key/token/secret/password naming.','Public-prefixed variables are shipped to the browser in common frontend frameworks.','Keep only public values in NEXT_PUBLIC_/VITE_/PUBLIC_ variables; move secrets to server-only env vars.'))
        # Frontend build
