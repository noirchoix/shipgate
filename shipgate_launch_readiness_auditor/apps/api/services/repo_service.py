...
class RepoService:
    ...
    def detect_stack(self, root: Path, files: list[RepoFile]) -> list[str]:
        paths={f.path.lower() for f in files}; stack=[]
        if 'package.json' in paths: stack.append('Node')
        if any('svelte.config' in p for p in paths): stack.append('SvelteKit')
        if any(p.endswith('vite.config.ts') or p.endswith('vite.config.js') for p in paths): stack.append('Vite')
        if 'requirements.txt' in paths or 'pyproject.toml' in paths: stack.append('Python')
        if 'main.py' in paths or any('/routers/' in p for p in paths): stack.append('FastAPI')
        if any('dockerfile' == Path(p).name for p in paths): stack.append('Docker')
        if any('.github/workflows/' in p for p in paths): stack.append('GitHub Actions')
        if 'vercel.json' in paths: stack.append('Vercel')
